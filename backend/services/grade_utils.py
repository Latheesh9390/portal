...
Grading scale (marks out of 100):
  91-100 → S   (Super/Outstanding) GP=10
  81-89  → A (Excellent)         GP=9
  71-79  → B  (Very Good)         GP=8
  61-69  → C (Good)              GP=7
  51-59  → D  (Above Average)     GP=6
  41-49  → E   (Pass)              GP=5
  <40    → F   (Fail)              GP=0
...
def compute_cgpa(db, hallticket):
    from models.regular_result import RegularResult

    results = (
        db.query(RegularResult)
        .filter(RegularResult.hallticket == hallticket)
        .all()
    )
